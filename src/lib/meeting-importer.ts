  ...
  try {
    ...
    if (meetingDate) {
      ...
      if (!dateFolder) {
        ...
        // Convert to YYYYMM integer, then negate for reverse chronological order
        // e.g., March 2026 → 202603 → -202603 (most recent folders first)
        // This avoids INT4 overflow from using millisecond timestamps
        const yearMonth = meetingDate.getFullYear() * 100 + (meetingDate.getMonth() + 1)
    ...
    // Build enriched content that includes summary and structured data for search
    const enrichedContent = `## Summary
${analyzed.rawSummary}

${analyzed.actionItems?.length ? `## Action Items
${analyzed.actionItems.map(item => `- ${item}`).join('\n')}

` : ''}${analyzed.decisions?.length ? `## Decisions
${analyzed.decisions.map(item => `- ${item}`).join('\n')}

` : ''}${analyzed.nextSteps?.length ? `## Next Steps
${analyzed.nextSteps.map(item => `- ${item}`).join('\n')}

` : ''}## Full Meeting Notes
${analyzed.fullContent}`
    ...
    const document = await prisma.knowledgeDocument.create({
      data: {
        ...
        tags: JSON.stringify([
          ...
          ...(analyzed.topics || []),
          // Add searchable tags for action items and decisions
          ...(analyzed.actionItems?.length ? ['has-action-items'] : []),
          ...(analyzed.decisions?.length ? ['has-decisions'] : []),
